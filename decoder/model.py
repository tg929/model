...
class GPT(nn.Module):
    ...
    def beam_search_generate(self, idx, tokenizer, max_new_tokens, beam_width=5,
        ...
        sep_id = tokenizer.sep_token_id if not is_simulation else None
        init_inference = True
        for step in range(max_new_tokens):
            candidates = []
            for score, seq, ended in beam:
                ...
                if init_inference or not kv_cache:
                    inference_res, init_inference = self(seq, tokenizer, kv_cache=kv_cache), False
                else:
                    # 取最后一个token进行推理
                    last_token = seq[:, -1:]
                    inference_res = self(last_token, tokenizer, kv_cache=True, current_idx=seq.shape[1] - 1)
                ...
                for i in range(beam_width):
                    ...
                    if linker:
                        ...
                        if new_token.item() in {eos_id, sep_id}:
                            new_ended = True
